...
public class ResourcePlanningApp {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Initialisation du bloc d'encodage Java par défaut (comme demandé)
            try { 
                System.setOut(new PrintStream(System.out, true, StandardCharsets.UTF_8)); 
                System.setErr(new PrintStream(System.err, true, StandardCharsets.UTF_8)); 
        ...
        try {
            ...
            // Récupération du cookie HR Center
            String hrCookie = props.getProperty("hr.cookie");
            if (hrCookie == null && props.getProperty("hr.phpsessid") != null) {
                hrCookie = "PHPSESSID=" + props.getProperty("hr.phpsessid");
            }
            ...
            System.out.println("[5/7] Connexion dynamique au HR Center & Calcul des absences/présences...");
            // loadHRData ne jette plus d'exception critique, elle gère ses erreurs en interne.
            Map<String, Integer> absencesJanvier = service.loadHRData(hrCookie, data.planning);

            System.out.println("[6/7] Calcul de la tension et identification des alertes de capacité...");
            // Calcul basé sur 22 jours ouvrés pour Janvier
            ResourcePlanningService.CapacityAlerts capacityAlerts = service.calculateCapacityAlerts(
                    jqlOpen,
                    teamSpecs,
                    absencesJanvier, // Les absences sont déjà traitées dans data.planning par loadHRData
                    22.0
            );
